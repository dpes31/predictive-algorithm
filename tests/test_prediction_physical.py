...
class PhysicalPredictionIntegrationTests(unittest.TestCase):
    ...
    def test_m4_is_computed_but_not_deployed_in_research_state(self):
        result = self._run()
        ...
        self.assertTrue(result.metadata["physical_evidence"]["active"])
        self.assertGreaterEqual(result.metadata["physical_evidence"]["matched_contexts"], 1)
    ...
    def test_physical_prediction_is_deterministic(self):
        ...
        self.assertEqual(
            [candidate.numbers for candidate in first.candidate_sets],
            [candidate.numbers for candidate in second.candidate_sets],
        )
